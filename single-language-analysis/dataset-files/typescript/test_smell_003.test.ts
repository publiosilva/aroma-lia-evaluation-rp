...
describe('UserTest', () => {
    it("test_smell_003", async () => {
        const name: string = "pessimisticRead";

        const id: number = createUser(name);

        const em = factory.createEntityManager();
        const tx = em.getTransaction();

        tx.begin();
        const u1 = em.find(User, id, LockModeType.PESSIMISTIC_READ);
        expect(u1).not.toBeNull();

        const executor = new TransactionExecutor(factory);
        const t = executor.asyncExe(s => {
            const user = s.find(User, id, LockModeType.PESSIMISTIC_READ);
